...
export const getLedgerHistory = createServerFn({ method: 'GET' }).handler(async () => {
  const user = await getSessionUser()
  if (!user) throw new Error('Authentication required')
  const db = getDb()
  const ownedAccounts = await db.select({ id: ledgerAccounts.id }).from(ledgerAccounts).where(eq(ledgerAccounts.ownerUserId, user.id))
  ...
  const transactions = new Map<string, { id: string; eventType: string; description: string; effectiveAt: string; lines: Array<{ account: string; debit: string; credit: string }> }>()
  for (const row of rows) {
    ...
    transaction.lines.push({ account: row.accountCode.split(':').at(-1) ?? row.accountCode, debit: row.debit, credit: row.credit })
